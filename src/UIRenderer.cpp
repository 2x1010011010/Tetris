...
UILayout::UILayout() : windowSize(GameConfig::WINDOW_WIDTH, GameConfig::WINDOW_HEIGHT),
...
UIRenderer::UIRenderer(sf::RenderWindow& window) : window_(window),
...
void UIRenderer::renderPlayfield(const GameController& controller) const
{
    ...
    for (int y = 0; y < PlayField::HEIGHT; ++y)
    {
        for (int x = 0; x < PlayField::WIDTH; ++x)
        {
            if (playfield.getCell(x, y) == Cell::Filled)
            {
